...
class IC(Component):
    ...
    def on_lightPort(self):
        ...
        self.logger.info("GamelightState: \n%s\n", pprint.pformat(self.GameLightState))
        ICL =  {'IC' : self.myActor, #intersection controller lights
               'GameLightState' : self.GameLightState}
        self.pubLightLog.send_pyobj(ICL)
        
    def on_densityPort(self):
        msg = self.densityPort.recv_pyobj()
        #Handle error if msg has bad data
        #assert isinstance(msg, dict), "msg is not a dict: %r" % msg
    ...
    #     self.logger.info("on_setLightsPort, waiting for reply")
    ...
    def on_setLightStatus(self):
        msg = self.setLightStatus.recv_pyobj()
        self.logger.info("Server response received: %s", pprint.pformat(msg))
        self.initialized = True  
        
    def setGameLights(self):
        #self.logger.info("setGameLights: %s", pprint.pformat(self.LightState))
        self.setLightsPort.send_pyobj(self.LightState)
        #=======================================================================
        # self.logger.info("setGameLights")
        # while self.pending > 0:     # Handle the case when there is a pending request          
        #     self.on_setLightsPort()
        # self.setLightsPort.send_pyobj(self.LightState)
        # self.pending += 1 
